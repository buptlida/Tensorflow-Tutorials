...
train_y  = model(X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden)
cost     = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(train_y, Y))
train_op = tf.train.RMSPropOptimizer(0.001, 0.9).minimize(cost)


test_y       = model(X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden)
test_indices = get_test_index(teX)
test_check   = tf.equal(tf.argmax(test_y, 1), tf.argmax(teY[test_indices], 1))
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        test_result = sess.run(test_op, feed_dict={X: teX[test_indices], Y: teY[test_indices], p_keep_conv: 1.0, p_keep_hidden: 1.0})
        print(i, test_result)
